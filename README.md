...
For example if the broker is in `us-west-2` region, the scope is `us-west-2/kafka-cluster`. It must be the same scope
...
All the query parameters calculated earlier are added to the authentication payload json as lower case strings (not
 uri encoded):
* `"action"`
* `"x-amz-algorithm"`
* `"x-amz-credential"`
