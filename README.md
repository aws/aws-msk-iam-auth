...
### Failure during re-authentication due to session name change (EKS Pod Identity/Other Credential Providers)

Problem: When using AWS credential providers that generate dynamic session names (e.g. EKS Pod Identity) with AWS MSK IAM authentication, the random STS session name generated by these credential providers causes a hard failure during re-authentication. MSK expects the same principal across re-authentication cycles, but when credentials are refreshed with a new session name, the client receives: 
```
Cannot change principals during re-authentication
```
Workaround: Configure a static session name using explicit JAAS configuration:

1. Create a dedicated IAM role for MSK access with appropriate permissions for your use case 
2. Add a trust policy to this role allowing your current role, such as your EKS Pod's assigned role to assume it:
  ...
  "Statement": [
    {
      "Effect": "Allow",
      "Principal": {
        "AWS": "arn:aws:iam::123456789012:role/YourCurrentCredentialProviderRole"
      },
...
```
3. In your Kafka client configuration, explicitly set the sasl.jaas.config:
