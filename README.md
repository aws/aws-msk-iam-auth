...
If your Kafka Client, producer or consumer, is running on EKS, you can use EKS service accounts to distribute IAM credentials. The [EKS service account documentation](https://docs.aws.amazon.com/eks/latest/userguide/iam-roles-for-service-accounts.html) is a good place to start. Following document covers one common scenario of cross account IAM access with EKS service account. Some folks had difficulty in making it work. The following documentation acts as a reference to validate . Our set-up uses VPC peering for cross account network access, and managed EC2 node group on EKS side. Each step below is linked AWS documentation for more details and troubleshooting:
